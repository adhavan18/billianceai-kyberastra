...
def gmail_authenticate():
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            flow = InstalledAppFlow.from_client_secrets_file('A:\Shoplifting-Detection\Shoplifting-Detection\Billiance_Gmail_API_Cred.json', SCOPES)
...
def send_email(video_filename):
    ...
    email_message = create_message_with_attachment(
        to="hamsavardhini.m@gmail.com",  # Updated recipient email!!!!!!!!!!!!!!!!!
...
# Initialize the video writer for detected segments
fourcc = cv2.VideoWriter_fourcc(*'mp4v')
video_writer = None
recording = False
record_frames = 100  # Number of frames to record after detection
    ...
    if success:
        ...
        for box, confidence, track_id in zip(boxes, confidences, track_ids):
            ...
            if confidence_history[track_id] is not None:
                ...
                # If confidence drop exceeds the threshold, start recording
                if confidence_drop > CONFIDENCE_DROP_THRESHOLD and not recording:
                    video_filename = f"shoplifting_video_{frame_count}.mp4"
                    video_writer = cv2.VideoWriter(video_filename, fourcc, fps, (width, height))
                    recording = True
                    recorded_count = 0
                    print("Shoplifting detected, recording video...")

                if recording:
                    video_writer.write(annotated_frame)
                    recorded_count += 1

                # Stop recording after capturing enough frames
                if recording and recorded_count > record_frames:
                    video_writer.release()
                    recording = False
                    print("Video recording completed, sending email...")
